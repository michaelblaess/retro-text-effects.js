...
**overflow** - shipped as a single, dependency-free file you can drop into any page.

Every effect runs on a plain `<pre>` block by rewriting its text content. There is
no canvas: the text stays selectable, box-drawing characters stay aligned, and the
colour is inherited from your element (perfect for an already-green console).
